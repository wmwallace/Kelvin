...
An on-device photo editor for Mac and iPhone. You drop in a photo (RAW, JPEG, HEIC or PNG),
Apple's Vision framework reads the scene, and the app hands back three or four fully-formed
...
The scene reader sees a ~768px proxy and never the RAW. This turns perception from a
...
| Render | Core Image built-in filters and colour cubes | Same memory as the decoder, no copy; no custom Metal |
| Perception | Apple Vision (`VisionPerceptionProvider`) | Ships with the OS, deterministic, a tenth of a second a read; measured better than the retired 2B model on both corpora (D27) |
...
**Platforms: Mac and iPhone (owner direction, 22 September 2026).** This section used to say
"Metal shaders do not port, so this is a Mac-only app". The audit that day found there is no
custom Metal in the repository at all — every effect is a Core Image built-in — and KelvinCore
compiled for iOS unmodified. The iPhone app is `Apps/KelvinPhone` (generated by
`scripts/make-iphone-project.sh`); it composes candidates through `ShippedCandidates`, the same
path the harness scores. Apple platforms only: the stack is Core Image, Vision and SwiftUI, and
that is positioning, not a limitation.
...
4. **Perception layer** — wire in a scene reader (a VLM until D27, Apple Vision since) to produce the perception JSON that step 3
   already consumes.
