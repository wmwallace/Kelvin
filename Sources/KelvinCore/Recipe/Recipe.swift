...
public struct Mask: Codable, Equatable, Sendable {
    ...
    /// A region grown outward from one clicked point until the picture stops looking like what was
    /// clicked — the magic wand. Another mask SOURCE, and the contiguous counterpart to `selection`:
    /// a colour range takes every matching pixel in the frame, this takes the one connected thing
    /// you pointed at. Clicking one sea stack selects that stack, not every dark rock on the beach.
    ///
    /// **A seed and a tolerance, never a bitmap** (RECIPE-SCHEMA #6, the same rule `shape` and
    /// `stamps` follow). The pixels are regenerated by `RegionGrow` at whatever resolution is being
    /// rendered, which is the only reason this can work at all: a region traced on a 1200 px proxy
    /// and stored as pixels would have to be resampled up to a 60 MP export, and a resampled
    /// selection edge is precisely the soft halo the feather work spent a session removing.
    ///
    /// It also has to be parametric to survive export *at all*. The export path builds its mask
    /// bitmaps from `LocalMasks.measure` and does not carry the app's preview bitmaps, and a mask
    /// the renderer is handed no bitmap for is silently skipped — so a bitmap-backed wand would
...
/// way every other mask here is stored: as the numbers needed to make it again, not as the pixels.
///
/// `RegionGrow` does the growing; this is only the record of what to ask it for.
public struct RegionSeed: Codable, Equatable, Sendable {
    /// The clicked point, normalised, **top-left origin** — matching `HealSpot`, `BrushStamp` and
    /// `MaskShape`, and matching what the app's `viewToNorm` hands back. Vision's boxes are the odd
    /// one out at bottom-left, and mixing the two is a bug this codebase has already had once.
    public var x: Double
    public var y: Double
    /// How different from the seed colour a pixel may be and still join, 0…1 as a fraction of the
    /// largest possible difference. Around 0.10 picks a sea stack out of the sky; 0.30 takes most
    /// of a beach.
    public var tolerance: Double
    /// Fraction of the tolerance the edge fades over. A hard boundary upscales from the working
    /// grid to an export as visible stair-stepping.
    public var softness: Double

    public init(x: Double, y: Double, tolerance: Double = 0.10, softness: Double = 0.25) {
        self.x = x; self.y = y; self.tolerance = tolerance; self.softness = softness
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        tolerance = clamp(try c.decodeIfPresent(Double.self, forKey: .tolerance) ?? 0.10, to: 0.001...1)
        softness = clamp(try c.decodeIfPresent(Double.self, forKey: .softness) ?? 0.25, to: 0...1)
    }

    enum CodingKeys: String, CodingKey { case x, y, tolerance, softness }
}
...
public struct BrushStamp: Codable, Equatable, Sendable {
    ...
    /// Take coverage AWAY rather than adding it.
    ///
    /// Without this a mask could only ever grow. Every mask in the app is built by union — brush
    /// dabs, a Vision subject, a flood fill — so one that grabbed too much could not be corrected
    /// by hand at all, and the only remedy was to delete it and start again. That is the whole
    /// reason `RegionGrow` leaking off a sea stack into Haystack Rock was a dead end rather than a
    /// rough edge: there was no way to say "not that part".
    ///
    /// Per STAMP rather than per stroke, because the photographer holds a modifier and keeps
    /// painting — the stroke is a sequence, and the order within it is what makes "add, then take
    /// a bit back" mean anything. `Renderer.brushMask` therefore applies stamps in order.
    ///
    /// **Absent means false, and false is not written.** Every existing recipe decodes byte for
    /// byte as it did (`decodeIfPresent`), and re-saving an add-only mask produces the same JSON as
    /// before rather than sprouting `"erase": false` on every dab of a 1200-stamp stroke. That also
    /// keeps the one real compatibility risk contained: an older build reading a recipe it does not
    /// understand would treat an erase stamp as an add stamp and draw the mask too large, so the
    /// key only ever appears in recipes that genuinely use it.
    public var erase: Bool

    public init(x: Double, y: Double, radius: Double, hardness: Double = 0.5, erase: Bool = false) {
        self.x = x; self.y = y; self.radius = radius; self.hardness = hardness; self.erase = erase
    }
    ...
    /// Hand-written so `erase: false` can be omitted — see the note on `erase`. **A new field must
    /// be added here as well as to `CodingKeys` and `init(from:)`, or it will silently never save.**
    /// That is not hypothetical: `GlobalAdjustments.fusion` was lost exactly this way, which is why
    /// `RecipeRoundTripTests` exists.
    public func encode(to encoder: Encoder) throws {
        var c = encoder.container(keyedBy: CodingKeys.self)
        ...
        if erase { try c.encode(true, forKey: .erase) }
    }

    enum CodingKeys: String, CodingKey { case x, y, radius, hardness, erase }
}
