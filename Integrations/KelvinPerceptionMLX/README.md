...
**The git repository contains no weights.** A source build downloads them once, at a pinned
revision; a release bundle ships them inside the app (see `docs/DECISIONS.md` D-model-4). The
...
was previously documented here as Apache-2.0. See `docs/DECISIONS.md` D-model-3.
...
The first *run* downloads the model (~1.6 GB) from Hugging Face and caches it.
